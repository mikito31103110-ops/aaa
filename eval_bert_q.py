...
def draw_overlay_one_page(
    ...
    for i in range(n):
        ...
        for m in metrics:
            draw.text(
                (cur_x - m["left"], target_top_y - m["top"]),
                m["text"],
                fill=(255, 255, 255),
                font=font,
            )
...
def main():
    ...
    ap.add_argument("--bert-lambda", type=float, default=1.0)
    ap.add_argument("--bert-passes", type=int, default=1)
    ap.add_argument("--topk", type=int, default=5)

    ap.add_argument("--pages", default="all")
    ap.add_argument("--out-dir", default="./eval_out_bert")
    ...
    print(f"Greedy+BERT  : {correct_bert / total_chars:.4f}")
    ...
    if args.stats:
        ...
        for ch, dct in counts.items():
            ...
            rows.append({
                ...
                "bert_err": b_err,
                "oracle_in_topK": orc,
                "greedy_miss_rate": ratio(g_err, total),
                "bert_miss_rate": ratio(b_err, total),
                "oracle_rate": ratio(orc, total),
...
PYTHONNOUSERSITE=1 python eval_bert.py \
  ...
  --box-width 4 \
  --out-dir ./outputs/picture/200017458/0.8
  --text-y-offset -3
"""