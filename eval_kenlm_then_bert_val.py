...
def make_float_grid(start: float, end: float, step: float) -> List[float]:
    if step <= 0:
...
def run_one_setting(
    ...
    if total_chars == 0:
        ...
        return {
            ...
            "greedy_acc": float("nan"),
            "kenlm_acc": float("nan"),
            "final_acc": float("nan"),
            "kenlm_lambda": kenlm_lambda,
            "bert_lambda": bert_lambda,
        }

    greedy_acc = correct_greedy / total_chars
    kenlm_acc = correct_kenlm / total_chars
    final_acc = correct_final / total_chars

    print(f"Greedy acc        : {greedy_acc:.4f}")
    print(f"Greedy+KenLM acc  : {kenlm_acc:.4f}")
    print(f"KenLM->BERT acc   : {final_acc:.4f}")
...
def main():
    ...
    # sweep
    ap.add_argument("--sweep-target", choices=["none", "bert", "kenlm", "both"], default="none")
    ap.add_argument("--sweep-start", type=float, default=0.2)
    ap.add_argument("--sweep-end", type=float, default=2.0)
    ap.add_argument("--sweep-step", type=float, default=0.2)
    ...
    vals = make_float_grid(args.sweep_start, args.sweep_end, args.sweep_step)
    rows = []

    print("\n[INFO] sweep mode enabled")
    print(f"[INFO] target = {args.sweep_target}")
    print(f"[INFO] values = {vals}")

    if args.sweep_target == "bert":
        for bert_lam in vals:
            child_out = out_dir / f"bert_lambda_{fmt_lambda(bert_lam)}"
            print("\n" + "=" * 80)
            print(f"[SWEEP] bert_lambda={bert_lam}  kenlm_lambda={args.kenlm_lambda}")
            res = run_one_setting(
                args=args,
                df=df,
                use_pages=use_pages,
                ...
                bert_model=bert_model,
                kenlm_lambda=float(args.kenlm_lambda),
                bert_lambda=float(bert_lam),
            )
            rows.append(res)

    elif args.sweep_target == "kenlm":
        for kenlm_lam in vals:
            child_out = out_dir / f"kenlm_lambda_{fmt_lambda(kenlm_lam)}"
            print("\n" + "=" * 80)
            print(f"[SWEEP] kenlm_lambda={kenlm_lam}  bert_lambda={args.bert_lambda}")
            res = run_one_setting(
                args=args,
                df=df,
                use_pages=use_pages,
                ...
                tokenizer=tokenizer,
                bert_model=bert_model,
                kenlm_lambda=float(kenlm_lam),
                bert_lambda=float(args.bert_lambda),
            )
            rows.append(res)

    elif args.sweep_target == "both":
        for kenlm_lam in vals:
            for bert_lam in vals:
                child_out = out_dir / f"kenlm_{fmt_lambda(kenlm_lam)}__bert_{fmt_lambda(bert_lam)}"
                print("\n" + "=" * 80)
                print(f"[SWEEP] kenlm_lambda={kenlm_lam}  bert_lambda={bert_lam}")
                res = run_one_setting(
                    args=args,
                    df=df,
                    use_pages=use_pages,
        ...
        raise ValueError(f"unknown sweep-target: {args.sweep_target}")

    if rows:
        df_sum = pd.DataFrame(rows)
        if "final_acc" in df_sum.columns:
            df_sum = df_sum.sort_values(
                by=["final_acc", "kenlm_acc", "greedy_acc"],
        ...
        print("\n===== SWEEP BEST =====")
        print(f"kenlm_lambda : {best['kenlm_lambda']}")
        print(f"bert_lambda  : {best['bert_lambda']}")
        print(f"greedy_acc   : {best['greedy_acc']:.4f}" if pd.notna(best["greedy_acc"]) else "greedy_acc   : nan")
        print(f"kenlm_acc    : {best['kenlm_acc']:.4f}" if pd.notna(best["kenlm_acc"]) else "kenlm_acc    : nan")
        print(f"final_acc    : {best['final_acc']:.4f}" if pd.notna(best["final_acc"]) else "final_acc    : nan")
...
PYTHONNOUSERSITE=1 python eval_kenlm_then_bert.py \
  ...
  --font-path /home/ihpc/Documents/saito/KODAI/fonts/IPAexGothic.ttf \
  --box-width 4 \
  --out-dir ./outputs/kenlm_then_bert/200017458_bert_sweep \
  --sweep-target bert \
  --sweep-start 0.2 \
  --sweep-end 2.0 \
  --sweep-step 0.2
